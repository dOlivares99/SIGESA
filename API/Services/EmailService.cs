...
public interface IEmailService
{
    ...
    Task EnviarConfirmacionReservaAsync(string destinatario, string nombreCliente, string tipoEvento, DateOnly fechaEvento, int numPersonas, decimal montoTotal);
}
...
public class EmailService : IEmailService
{
    ...
    public async Task EnviarConfirmacionReservaAsync(string destinatario, string nombreCliente, string tipoEvento, DateOnly fechaEvento, int numPersonas, decimal montoTotal)
    {
        var smtp = _config["Email:Smtp"] ?? "smtp.gmail.com";
        var puerto = int.Parse(_config["Email:Puerto"] ?? "587");
        var usuario = _config["Email:Usuario"] ?? throw new InvalidOperationException("Email:Usuario no configurado.");
            ...
            Body = $"""
                ...
                <body style="margin:0;padding:0;background:#f5f4ef;font-family:'Inter',Arial,sans-serif;">
                  <div style="max-width:480px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08);">
                    ...
                    <div style="padding:32px;">
                      <h2 style="color:#1a1a1a;margin:0 0 8px;font-size:1.1rem;">Hola, {nombreCliente}</h2>
                      <p style="color:#555;margin:0 0 24px;line-height:1.6;">
                        Tu reserva para <strong>{tipoEvento}</strong> ha sido <strong>confirmada</strong>. Aquí tienes el resumen:
                      </p>
                      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
                        <tr>
                          <td style="padding:8px 0;color:#999;font-size:.85rem;">Fecha del evento</td>
                          <td style="padding:8px 0;color:#1a1a1a;font-size:.85rem;text-align:right;">{fechaEvento:dd/MM/yyyy}</td>
                        </tr>
                        <tr>
                          <td style="padding:8px 0;color:#999;font-size:.85rem;">Número de personas</td>
                          <td style="padding:8px 0;color:#1a1a1a;font-size:.85rem;text-align:right;">{numPersonas}</td>
                        </tr>
                        <tr>
                          <td style="padding:8px 0;color:#999;font-size:.85rem;">Monto total</td>
                          <td style="padding:8px 0;color:#1a1a1a;font-size:.85rem;text-align:right;">{montoTotal:C}</td>
                        </tr>
                      </table>
                      <p style="color:#999;font-size:.8rem;margin:0;line-height:1.6;">
                        Si tienes alguna duda sobre tu reserva, contáctanos respondiendo a este correo.
                      </p>
                    </div>
                    <div style="background:#f8f7ff;padding:16px;text-align:center;">
                      <p style="color:#aaa;font-size:.75rem;margin:0;">SIGESA v1.0 © 2026</p>
